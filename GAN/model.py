...
class Finn(object):
    def __init__(self, sess, df_dim, batch_size, dropout_prob, l1_weight, ssim_weight, clipping_weight,
                 discriminator_weight, writer_path, video_path):
        self.df_dim = df_dim
        ...
        print(f'Height: {self.input_height}')
        print(f'Width: {self.input_width}')
        self.dataset_name = 'tom'
    ...
    def discriminator(self, triplet, phase, reuse=False):
        with tf.variable_scope("discriminator") as scope:
            ...
            h0 = lrelu(
                conv2d(triplet, self.df_dim, hh=4, ww=4, stride_w=2, stride_h=2, padding='VALID', name="d_h0_conv"))
            h1 = lrelu(
                bn(conv2d(h0, self.df_dim * 2, hh=4, ww=4, stride_w=2, stride_h=2, padding='VALID', name="d_h1_conv"),
                   phase, name="d_h1_bn"))
            h2 = lrelu(
                bn(conv2d(h1, self.df_dim * 4, hh=4, ww=4, stride_w=2, stride_h=2, padding='VALID', name="d_h2_conv"),
                   phase, name="d_h2_bn"))
            h3 = lrelu(
                bn(conv2d(h2, self.df_dim * 8, hh=4, ww=4, stride_w=2, stride_h=2, padding='VALID', name="d_h3_conv"),
                   phase, name="d_h3_bn"))
    ...
    def generator(self, doublet):
        with tf.variable_scope("generator"):
            ...
            for i, outputdepth in enumerate(self.gen_layer_depths):
                result = conv_block(current_input, self.is_training, self.gen_filter_sizes[i], outputdepth,
                                    name=('g_conv_block' + str(i)))
            ...
            for i, outputdepth in enumerate(rev_layer_depths[1:]):  # reverse process exactly until last step
                ...
                result = bilinear_resize_deconv_block(current_input, self.is_training, rev_filter_sizes[i], outputdepth,
                                                      name=('g_deconv_block' + str(i)))
                if i <= 4:
            ...
            h = bilinear_resize_tanh_deconv_block(current_input, self.is_training, rev_filter_sizes[-1], outputdepth,
                                                  name=('g_tanh_deconv'))
    ...
    def train(self, config):
        ...
        for epoch in range(config.epoch):
            ...
            for idx in range(0, batch_idx):
                ...
                if (idx == 0):
                    print(
                        f"Epoch: {epoch} {idx} time: {time.time() - start_time}, d_loss_total {errD_fake + errD_real}, g_loss {errG}, l1_loss {errG_l1}, ms_ssim_loss {errG_ssim}")
            ...
            if np.mod(epoch, 5) == 0:
                ...
                if (epoch == 0):
                    ...
                    [imsave(os.path.join(config.image_dir, "Z13_epoch%dimg%d.jpg" %
                                         (epoch, i)), (Z_imgs[i, :, :, :3] + Z_imgs[i, :, :, 3:]) / 2 + self.mean_img)
                     for i in range(Z_imgs.shape[0])]
    ...
    def test(self, config):
        ...
        [imsave(os.path.join(config.image_dir, "Z13_valimgno%d.jpg" %
                             (i)), (Z_imgs[i, :, :, :3] + Z_imgs[i, :, :, 3:]) / 2 + self.mean_img) for i in
         range(Z_imgs.shape[0])]
